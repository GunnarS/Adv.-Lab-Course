...
Holography is a technique that is used to record and reconstruct an image such that the image appears to be projected away from the surface of the film footage and into the position of the original object at the time of recording. One can distinguish holograms by the technique used to store them. In the case of \textit{amplitude holograms} the local transmittance of the material is changed, whereas the local optical path length is modulated in the case of \textit{phase holograms}. This can either be obtained by changing the thickness of the medium or the refractive index locally. The latter technique uses the \textit{photorefractive effect} which is explained in section \ref{subsec:theo_photorefractive}. Holograms can be further distinguished by the geometry used for recording and reconstruction of the holograms. These are compared in figure \ref{fig:theo_holograms}.
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{graphics/theo_holographs}
  \caption{Comparison of writing and reconstructing geometrys for transmission and reflection holograms}
  \label{fig:theo_holograms}
\end{figure}
 In order to record a hologram, coherent light must be split into a signal wave (that reflects off the object to be recorded) and a reference wave which is superposed onto the signal wave at the site of the film footage or crystal. The principle mode of operation of amplitude holograms can be demonstrated in the following way: Two interfering beams will produce an interference pattern with an intensity distribution as follows:
\begin{align}
I=|S+R|^2=|S|^2 + |R|^2 + S^* R + R^* S
\end{align}
Where $I$ is the intensity distribution, $R$ is the complex amplitude of the reference wave, and $S$ is the amplitude of the signal wave. This is the same situation as was discussed in the section about coherence and interference, where the interference term is composed of the mixing of the amplitudes of each respective wave. The interference term will give rise to the pattern of light and dark regions that are spatially projected onto the recording material. Once the film footage has been developed, the transmittance is related to the incident light according to:
\begin{align}
\tau = a-b \left| S+R \right|^2
\end{align}
When the footage is illuminated with the reference beam, the transmitted intensity becomes:
\begin{align}
...
\subsection{Photorefractive Effect}\label{subsec:theo_photorefractive}
...
\subsection{Thick Holograms and the Bragg Condition}\label{subsec:theo_bragg}
A hologram which is thick in comparison to the wavelength of the light used to record it, is called \textit{volume hologram}. In this case the \textsc{Bragg} condition 
\begin{align*}
\sin(\theta_\textrm{Bragg}) = \frac{\lambda}{2n\Lambda}
\end{align*}
has to be fulfilled in order to reconstruct the signal wave. Here $\theta_\textrm{Bragg}$ is the angle between the incoming wave and the normal to the hologram inside the medium, $n$ is the refractive index and $\Lambda = \frac{2\pi}{K}$ the grating constant of the material.

One can define the diffraction effiency $\eta$ of holograms
\begin{align*}
\eta = \frac{I_\textrm{diff}}{I_\textrm{diff} + I_\textrm{trans}}
\end{align*}
with $I_\textrm{diff}$ being the intensity of the diffracted and $I_\textrm{trans}$ the intensity of the transmitted beam. 

For the diffraction effiency in transmission geometry of phase holograms the \textsc{Kogelnik}'s law of coupled waves yields
\begin{align}
\eta = \nu^2 \frac{\sin^2\sqrt{\nu^2+\xi^2}}{\nu^2+\xi^2}
\label{eq:kogelnik}
\end{align}
where 
\begin{align*}
\xi = \frac{Kd}{2}\left(\theta - \theta_\textrm{Bragg}\right) \hspace{2cm} \nu = \frac{\pi d\Delta n_0}{\lambda \cos \theta}
\end{align*}
and $d$ being the thickness of the hologram, $\theta$ the angle between the incoming wave and the normal to the hologram inside the medium and $\Delta n_0$ the amplitude of the modulation of the refractive index. 

\subsection{Lithium Niobate}
Lithium niobate (\linbo) is a ferroelectric and therefore also pyro- and piezoelectric crystal. Figure \ref{fig:linbo} sketches the crystal structure of \linbo below (left) and above (right) its \textsc{Curie} temperature of about $T_\mathrm{C} = \unit[1210]{^\circ C}$.
\begin{figure}[H]
    \begin{minipage}{0.2\textheight}
  \includegraphics[width=1.0\textwidth]{graphics/linbo_para}
    \end{minipage}
    \hspace{1.3cm}
    \begin{minipage}{0.2\textheight}
  \includegraphics[width=1.0\textwidth]{graphics/linbo_ferro}
    \end{minipage}
  \caption{Para- and ferroelectric phase of \linbo. Double cross-hatched circles denote lithium atoms, single cross-hatched ones niobium and big white circles depict oxygen atoms. ~\cite{weis}}
  \label{fig:linbo}
\end{figure}
As one can see, the \linbo crystal consists of planar sheets of oxygen atoms in a distorted hexagonal close-packed configuration. Furthermore the interstices are occupied one-third by lithium ions, one-third by niobium ions and one-third are vacant. The order is depicted in the figure above.

Indeed the positive ions in the interstices are displaced from their position of inversionsymmetry at temperatures below the \textsc{Curie} temperature (see figure \ref{fig:linbo} right). This leads to a non-vanishing spontaneous polarisation of the crystal changing with temperature as also the displacement in the elementary cell changes with temperature (primary pyroelectricity). Another contribution occurs due to thermal expansion of the crystal, because it is also piezoelectric (secondary pyroelectricity).
\linbo is even a ferroelectric crystal which just means that its direction of polarisation is revertible by an external electric field. If not insulated the change of the spontaneous polarisation due to temperature change can be observed as a current.

In the present case the crystal is additionaly doped with iron. The electrons from the Fe$^{2+}$-impurities are being excited into the conduction band by illumination, where they migrate because of miscellaneous transport mechanisms. In that way they cause the photorefractive effect of the crystal (see also section \ref{subsec:theo_photorefractive}).
...
\begin{figure}[H]
  ...
  \includegraphics[width=1.0\textwidth]{graphics/transmiss1}
    \end{minipage}
    \hspace{1.3cm}
    \begin{minipage}{0.175\textheight}
...
Using an expanding agent indeed promises an even better result than simple reflection holography, because after the expansion the coating contracts again such that the lattice constant gets smaller. Therefore it is possible to see the hologram at shorter wavelength. So we expect that it is not possible to reconstruct it with the red laser light we used. Instead it should be possible to observe the hologram with a bulb of high intensity emitting white light, because the \textsc{Bragg} condition will just "choose" the right wavelength as described in section \ref{subsec:theo_bragg}. But in practise a big problem arose because of a very long drying time. In fact the expanded plate was not dry after more than 22 hours. We are of the opinion that this should have happened because of wet desiccant. Anyway it of course was not possible to take a holograph with a wet holographic plate. So unfortunately we could not confirm the above mentioned indications.
...
Our data is presented in table \ref{tab:ana_rocking} in the appendix. In figure \ref{fig:ana_rocking} the diffraction effiency $\eta$ is plotted versus $\theta$. We fit a function of the form (\ref{eq:kogelnik}) to the data. The fit yields:
\begin{align}
...
\resizebox{0.8\textwidth}{!}{
	\input{graphics/ana_rocking.tex}
}
...
\section{Abstract}
First of all, we successfully recorded the transmission hologram and the reflection hologram without using the expanding agend. Unfortunately we missed to remove the OD filter while exposing the holographic plate. Therefore our hologram is weaker than it was supposed to be.

On account of a wet holographic plate we were not able to successfully use the expanded plate.

Furthermore our writing curve shows an unexpected behaviour. No fit with reasonable parameters can be performed, therefore we reject it.

Finally our measured erasing and \textsc{Rocking} curve are quite nice and show all the expected characteristics.
