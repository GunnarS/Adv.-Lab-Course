...
Indeed the positive ions in the interstices are displaced from there position of inversionsymmetry at temperatures below the \textsc{Curie} temperature (see figure \ref{fig:linbo} right). This leads to a non-vanishing spontaneous polarisation of the crystal changing with temperature as also the displacement in the elementary cell changes with temperature (primary pyroelectricity). Another contribution occurs due to thermal expansion of the crystal, because it is also piezoelectric (secondary pyroelectricity).
\linbo is even a ferroelectric which just means that its direction of polarisation is revertible by an external electric field. If not insulated the change of the spontaneous polarisation due to temperature change can be observed as a current.
