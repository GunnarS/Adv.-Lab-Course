...
\end{figure}
We adjust the left SCA to accept every event above the 511keV peak, because all these events originate from the 1275keV photon of the decay of \Ne. So the left detector yields the start signal for the TAC. The right SCA stays unchanged and accepts every event in the 511keV peak. The right detector therefore yields the stop signal for the TAC. Again we check the signals of coincidence unit and TAC for simultaneousness with an oscilloscope.
...
\end{align}
This value is near the value given in \cite{paper}:
\begin{align}
...
\end{align}
We couldn't find any values in literature to compare the obtained values to. At least it seems to be reasonable that the lifetime of bound positronium states is much bigger than the lifetime of free positrons in the crystal.
\section{Abstract}
